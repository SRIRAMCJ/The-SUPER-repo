...
test('scheduler dispatch contract exposes lease fencing identity for transport',()=>{const registry=new RemoteWorkerRegistry();registry.register({workerId:'dispatch-a',capabilities:['runtime.execute']});const leases=new RemoteWorkerLeaseManager();const scheduler=new RemoteWorkerScheduler({registry,leases});const result=scheduler.schedule({executionId:'exec-dispatch',capabilityId:'runtime.execute'});assert.equal(result.state,'scheduled');assert.ok(result.leaseId);assert.ok(result.fencingToken);assert.equal(leases.get(result.leaseId).capabilityId,'runtime.execute');});
