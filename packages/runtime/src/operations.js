...
export class RuntimeOperations {
  ...
  async diagnostics() {
    ...
    try {
      ...
      add('control-plane.health', health.status === 'healthy' ? 'info' : 'error', health.status === 'healthy' ? 'pass' : 'fail', `Runtime health is ${health.status}`, { status: health.status, activeExecutions: health.activeExecutions, recentFailures: health.recentFailures });
    } catch (error) {
      add('control-plane.health', 'error', 'fail', 'Runtime health check failed', { error: errorMessage(error) });
    }
    ...
    try {
      const metrics = cp.observability.getMetrics();
      const eventCount = cp.observability.getEvents().length;
      const traceCount = cp.observability.getTraces().length;
      add('observability.retention', 'info', 'pass', 'Bounded observability data is readable', { retainedEvents: eventCount, retainedTraces: traceCount, activeExecutions: metrics.activeExecutions });
    } catch (error) {
      add('observability.retention', 'error', 'fail', 'Observability retention could not be inspected', { error: errorMessage(error) });
    }
    ...
    return Object.freeze(structuredClone({
      ...
      status,
      summary: { checks: checks.length, passed: checks.filter((check) => check.status === 'pass').length, failed: failures.length, warnings: warnings.length },
      runtime: { version: this.runtimeVersion, nodeVersion: this.nodeVersion, platform: this.platform },
      operations: { count: this.getOperations().length, controlOperations: this.getOperations().filter((item) => item.classification === 'control').length },
      checks
    }));
  }
}

function operation(id, method, path, classification, description) {
  return { id, method, path, classification, description, schemaVersion: SCHEMA_VERSION };
}

function checkComponent(checks, id, component, methods, optional = false) {
