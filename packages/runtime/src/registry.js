export class CapabilityRegistry {
  ...
  register(manifest, handler) {
    ...
    if (typeof handler !== 'function') throw new TypeError(`Executable handler required for ${manifest.id}`);
...
export function validateCapability(value) {
  ...
  if (!['agent','skill','tool','command','workflow','team','mission','model','memory','artifact','plugin','policy','evaluation','benchmark','event','execution'].includes(value.kind)) throw new Error(`Invalid capability kind: ${value.kind}`);
