...
export class WorkflowEngine {
  ...
  async execute(workflow, input = {}, context = {}) {
    ...
    try {
      for (const step of workflow.steps ?? []) {
        ...
        const result = this.planBuilder ? await this.executePlannedStep(step, entry, current, context) : await this.executionEngine.execute(step.capability, current, context);
  ...
  async executePlannedStep(step, entry, input, context) {
    const plan = this.planBuilder.build({ capabilityId: step.capability, domain: context.domain ?? entry.manifest.domain ?? undefined });
    if (!plan.ok) return { status: 'failed', error: plan.error, plan };
    const result = await this.planExecutor.execute(plan, input, { ...context, workflowStep: step.id ?? step.capability });
    return { ...result, plan };
  }
}
