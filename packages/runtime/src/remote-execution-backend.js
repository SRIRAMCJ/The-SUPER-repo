...
export class RemoteExecutionBackend {
  constructor({ transport, backendId = 'remote' } = {}) {
    if (!transport || typeof transport.execute !== 'function') {
      throw new TypeError('RemoteExecutionBackend requires a transport with execute()');
    }
  ...
  async execute({ executionId, input = {}, context = {}, capability } = {}) {
    if (typeof executionId !== 'string' || !executionId.trim()) {
      return failed(executionId, 'INVALID_EXECUTION_ID', 'Remote execution requires an execution id');
    }
    ...
    try {
      ...
      if (context.signal?.aborted) {
        return {
          ...
          error: { code: 'CANCELLED', message: errorMessage(context.signal.reason ?? error), retryable: true },
        };
      }
      return {
        executionId,
        status: 'failed',
        backend: this.backendId,
        error: { code: error?.code ?? 'REMOTE_EXECUTION_FAILED', message: errorMessage(error), retryable: error?.retryable === true },
      };
    }
  }
}
...
function normalizeResult(executionId, backend, result) {
  if (!result || typeof result !== 'object') {
    return failed(executionId, 'INVALID_REMOTE_RESULT', 'Remote execution transport returned an invalid result');
  }
  const status = result.status ?? (result.ok === true ? 'succeeded' : 'failed');
  if (!['succeeded', 'failed', 'cancelled', 'timed_out'].includes(status)) {
    return failed(executionId, 'INVALID_REMOTE_STATUS', 'Remote execution transport returned an unsupported status');
  }
  return {
    executionId,
    status,
...
function failed(executionId, code, message) {
  return { executionId, status: 'failed', backend: 'remote', error: { code, message, retryable: false } };
}
