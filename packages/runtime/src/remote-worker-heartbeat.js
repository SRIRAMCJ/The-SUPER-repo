...
export class RemoteWorkerHeartbeatMonitor {
  ...
  constructor({ registry, clock = () => Date.now(), timeoutMs = 30_000, leaseManager = null, failoverController = null, onExecutionLost = null } = {}) {
    ...
    if (failoverController !== null && typeof failoverController.handleExecutionLost !== 'function') throw new TypeError('failoverController must expose handleExecutionLost()');
        ...
        if (this.#leaseManager) {
          for (const lease of this.#leaseManager.list()) {
            ...
            if (this.#failoverController) {
              try {
                reassignment = await this.#failoverController.handleExecutionLost({
                ...
                reassignmentError = {
                  code: error?.code ?? 'REMOTE_FAILOVER_FAILED',
