...
test('does not mark fresh workers unhealthy',()=>{let now=Date.parse('2026-09-20T00:00:00Z');const r=new RemoteWorkerRegistry({clock:()=>new Date(now)});r.register({workerId:'w1'});now+=10_000;const m=new RemoteWorkerHeartbeatMonitor({registry:r,clock:()=>now});assert.equal((await m.sweep()).changed.length,0);});
