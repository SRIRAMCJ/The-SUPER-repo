...
export class TaskGraphExecutor {
  constructor({ executeTask, events = null, clock = () => new Date(), stateStore = null } = {}) {
  ...
  async #run(plan, executionId, startedAt, input, context, config, results, attempt, resumed) {
    ...
    const runTask = async (task) => {
      ...
      try {
        ...
        await persist({ status: normalized.status === 'succeeded' ? 'running' : 'failed', currentTaskId: task.id, results: orderedResults(plan, results), error: normalized.error ?? null, attempt });
        ...
        results.set(task.id, normalized);
        await persist({ status: 'failed', currentTaskId: task.id, results: orderedResults(plan, results), error: normalized.error, attempt });
    ...
    try {
      ...
      for (const id of pending) results.set(id, { status: 'skipped', error: { code: config.failFast ? 'TASK_FAIL_FAST' : 'TASK_NOT_SCHEDULED', message: `Task was not started: ${id}`, retryable: false } });
      const output = finish(plan, executionId, startedAt, config, results, resumed, attempt, this.clock);
      await persist({ status: output.status === 'succeeded' ? 'succeeded' : 'failed', results: output.results, output: output.output ?? null, result: output, error: output.error ?? null, finishedAt: output.finishedAt, attempt, currentTaskId: null });
      return output;
    } catch (error) {
      const normalized = normalizeError(error);
      const output = { schemaVersion: '0.1.0', type: 'task-graph-execution', executionId, goal: plan.goal, startedAt, finishedAt: this.clock().toISOString(), status: 'failed', strategy: config.strategy, maxConcurrency: config.maxConcurrency, failFast: config.failFast, rootTaskId: plan.order.at(-1), results: orderedResults(plan, results), error: normalized, resumed, attempt };
      await persist({ status: 'failed', results: output.results, result: output, error: normalized, finishedAt: output.finishedAt, attempt });
...
function finish(plan, executionId, startedAt, config, results, resumed, attempt, clock) {
  ...
  const output = { schemaVersion: '0.1.0', type: 'task-graph-execution', executionId, goal: plan.goal, startedAt, finishedAt: clock().toISOString(), status: failed || skipped ? 'failed' : 'succeeded', strategy: config.strategy, maxConcurrency: config.maxConcurrency, failFast: config.failFast, rootTaskId: plan.order.at(-1), output: rootResult?.output ?? null, results: ordered, resumed, attempt };
  return output;
}
