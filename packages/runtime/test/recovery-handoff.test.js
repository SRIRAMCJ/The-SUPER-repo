...
test('handoff follows offer, accept, execute and fenced completion lifecycle', () => {
  ...
  const accepted=k.accept({transactionId:'tx1',targetNodeId:'node-b',ownerId:'owner-b'});
...
test('wrong target and stale fencing token cannot execute or complete a handoff', () => {
  const lease=new RecoveryLeaseKernel({leaseTtlMs:5000}); const k=new RecoveryHandoffKernel({lease});
  k.offer({transactionId:'tx2',sourceNodeId:'a',targetNodeId:'b'}); const accepted=k.accept({transactionId:'tx2',targetNodeId:'b',ownerId:'o'});
...
test('second active handoff is rejected and terminal handoff cannot be reused', () => {
  const lease=new RecoveryLeaseKernel({leaseTtlMs:5000}); const k=new RecoveryHandoffKernel({lease});
  ...
  const accepted=k.accept({transactionId:'tx3',targetNodeId:'b',ownerId:'o'}); k.beginExecution({transactionId:'tx3',targetNodeId:'b',fencingToken:accepted.handoff.fencingToken}); k.complete({transactionId:'tx3',targetNodeId:'b',fencingToken:1});
  assert.throws(()=>k.beginExecution({transactionId:'tx3',targetNodeId:'b',fencingToken:1}),e=>e.code==='RECOVERY_HANDOFF_NOT_ACCEPTED');
});

test('offer expires deterministically before acceptance', () => {
...
test('cancellation is fenced and auditable', () => {
  const lease=new RecoveryLeaseKernel({leaseTtlMs:5000}); const k=new RecoveryHandoffKernel({lease});
  k.offer({transactionId:'tx5',sourceNodeId:'a',targetNodeId:'b'}); const accepted=k.accept({transactionId:'tx5',targetNodeId:'b',ownerId:'o'});
