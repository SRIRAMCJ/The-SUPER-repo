...
test('remote backend submits a normalized provider-neutral execution request', async () => {
  ...
  const backend = new RemoteExecutionBackend({
    transport: {
      async execute(request, context) {
        ...
        return { status: 'succeeded', output: { value: 42 }, remoteExecutionId: 'remote-42' };
      },
    },
  });
  ...
  const result = await backend.execute({
    ...
    input: { command: 'node', args: ['-e', 'process.stdout.write("ok")'] },
    context: { signal: controller.signal, timeoutMs: 2500 },
    capability: { id: 'runtime/remote-test', execution: { backend: 'remote' } },
  });
  ...
  assert.equal(calls[0].request.executionId, 'exec-remote-1');
  assert.equal(calls[0].request.schemaVersion, '0.1.0');
...
test('remote backend propagates cancellation without hiding the terminal state', async () => {
  const controller = new AbortController();
  const backend = new RemoteExecutionBackend({
    transport: {
...
test('remote backend converts transport failures into structured retry-aware failures', async () => {
  const backend = new RemoteExecutionBackend({
    transport: {
      async execute() {
        throw Object.assign(new Error('worker unavailable'), { code: 'REMOTE_UNAVAILABLE', retryable: true });
      },
    },
  });
...
test('remote backend validates command and arguments before transport dispatch', async () => {
  let calls = 0;
