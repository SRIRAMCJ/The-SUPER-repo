...
export class FaultInjectionEngine {
  ...
  register({
    ...
    if (!Number.isInteger(maxTriggers) || maxTriggers < 1) {
      throw new TypeError('maxTriggers must be a positive integer');
    }
    const scenario = Object.freeze({
      schemaVersion: FAULT_INJECTION_SCHEMA_VERSION,
  ...
  remove(scenarioId) {
    return this.#scenarios.delete(String(scenarioId));
  }
  ...
  async evaluate(context, phase, random = Math.random) {
    ...
    for (const [id, scenario] of this.#scenarios) {
      ...
      let matches = false;
      try {
        matches = Boolean(await scenario.match(context));
      } catch (error) {
        this.#record('matcher_error', id, phase, context, error);
        continue;
      }
  ...
  #record(type, scenarioId, phase, context, error) {
    const event = {
      ...
      scenarioId,
      phase,
      contextHash: fingerprintContext(context),
...
export class RecoveryController {
  ...
  constructor({ clock = () => Date.now(), maxHistory = 5000 } = {}) {
    ...
    if (!Number.isInteger(maxHistory) || maxHistory < 1) {
      throw new TypeError('maxHistory must be a positive integer');
    }
    this.#clock = clock;
    this.#maxHistory = maxHistory;
  }
  ...
  async recover(
    key,
    { operation, attempts = 3, backoffMs = 0, shouldRetry = () => true, onAttempt = null } = {}
  ) {
    const id = String(key);
    ...
    if (!Number.isInteger(backoffMs) || backoffMs < 0) {
      throw new TypeError('backoffMs must be a non-negative integer');
    }
    ...
    try {
      ...
      for (let attempt = 1; attempt <= attempts; attempt++) {
        ...
        try {
          if (onAttempt) await onAttempt({ attempt, recoveryId: state.recoveryId });
          const value = await operation({ attempt, recoveryId: state.recoveryId });
          ...
          if (attempt === attempts || !(await shouldRetry(error, attempt))) break;
      ...
      state.error = {
        ...
        message: lastError?.message ?? 'Recovery failed'
      };
      this.#record(state);
      return clone(state);
    } finally {
  ...
  inspect(key) {
    const state = this.#recoveries.get(String(key));
...
export async function withFaultInjection(
  operation,
  { faults = null, context = {}, recovery = null } = {}
) {
  if (typeof operation !== 'function') throw new TypeError('operation must be a function');
  if (faults !== null && !(faults instanceof FaultInjectionEngine)) {
    throw new TypeError('faults must be a FaultInjectionEngine');
  }
  if (recovery !== null && typeof recovery !== 'function') {
    throw new TypeError('recovery must be a function');
  }

  const checkpoint = async phase => {
    if (!faults) return [];
    const triggered = await faults.evaluate(context, phase);
    await applyTriggeredFaults(triggered, context);
    return triggered;
  };

  try {
    await checkpoint('before');
    const value = await operation({ context, checkpoint });
    ...
    return {
      outcome: result?.outcome === 'recovered' ? 'recovered' : 'failed',
      error,
      recovery: result
    };
  }
...
async function applyTriggeredFaults(triggered, context) {
  for (const event of triggered) {
    const fault = event.fault;
    if (typeof fault === 'function') {
      const result = await fault(context, event);
      if (result instanceof Error) throw result;
      continue;
    }
    if (fault instanceof Error) throw fault;
    if (fault === 'throw') throw new Error(`Injected fault: ${event.scenario.name}`);
  }
}

function publicScenario(scenario) {
  const { match, ...safe } = scenario;
  return Object.freeze({ ...safe });
}

function fingerprintContext(value) {
