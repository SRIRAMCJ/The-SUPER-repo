...
export class BenchmarkGovernanceEngine {
  constructor({ regressionEngine = new BenchmarkRegressionEngine(), store = new ExecutionStateStore(), eventBus = null, clock = () => new Date() } = {}) {
    ...
    if (!store || typeof store.create !== 'function' || typeof store.get !== 'function' || typeof store.update !== 'function') {
      throw new TypeError('BenchmarkGovernanceEngine requires a compatible state store');
    }
  ...
  async initialize(baselineRun, metadata = {}) {
    ...
    if (existing) throw conflict(`Benchmark baseline already exists: ${baselineRun.benchmarkSuiteId}`, 'BENCHMARK_BASELINE_EXISTS');
  ...
  async evaluate(candidateRun, gates = {}) {
    ...
    return Object.freeze({
      schemaVersion: '0.1.0',
      type: 'benchmark-governance-evaluation',
      benchmarkSuiteId: candidateRun.benchmarkSuiteId,
      baselineVersion: baseline.version,
      eligibleForPromotion: regression.status === 'passed',
      regression
    });
  }
  ...
  async promote(candidateRun, gates = {}, expectedVersion = null, metadata = {}) {
    ...
    if (expectedVersion !== null && (!Number.isInteger(expectedVersion) || expectedVersion < 1)) {
      throw new TypeError('expectedVersion must be a positive integer or null');
    }
    const regression = this.regressionEngine.compare(current.current, candidateRun, gates);
    if (regression.status !== 'passed') {
      await this.#emit('benchmark.baseline.promotion_rejected', { benchmarkSuiteId: candidateRun.benchmarkSuiteId, version: current.version, regression });
      return Object.freeze({
        schemaVersion: '0.1.0',
        type: 'benchmark-promotion',
        status: 'rejected',
        benchmarkSuiteId: candidateRun.benchmarkSuiteId,
    ...
    const nextHistory = [...current.history, {
      version: nextVersion,
      action: 'promoted',
      benchmarkRun: clone(candidateRun),
      regression: clone(regression),
      metadata: sanitizeMetadata(metadata),
      at: now
    }];
    const updated = await this.store.update(baselineId, {
      version: nextVersion,
      current: clone(candidateRun),
      history: nextHistory,
      updatedAt: now
    }, expectedVersion);
  ...
  async rollback(benchmarkSuiteId, targetVersion, expectedVersion = null) {
    ...
    if (expectedVersion !== null && (!Number.isInteger(expectedVersion) || expectedVersion < 1)) throw new TypeError('expectedVersion must be a positive integer or null');
    ...
    const updated = await this.store.update(id, {
      version: nextVersion,
      current: clone(target.benchmarkRun),
      history: [...current.history, { version: nextVersion, action: 'rollback', fromVersion: current.version, targetVersion, benchmarkRun: clone(target.benchmarkRun), at: now }],
      updatedAt: now
    }, expectedVersion);
    const result = Object.freeze({
      schemaVersion: '0.1.0',
      type: 'benchmark-rollback',
      status: 'rolled_back',
      benchmarkSuiteId,
      previousVersion: current.version,
      version: updated.version,
  ...
  async #emit(type, payload) {
    ...
    if (typeof this.eventBus.emit === 'function') await this.eventBus.emit(type, payload);
  }
}

function stateId(benchmarkSuiteId) { return `benchmark-baseline:${benchmarkSuiteId}`; }

function validateSuiteId(value) {
  if (typeof value !== 'string' || !value.trim()) throw new TypeError('benchmarkSuiteId must be a non-empty string');
}
...
function failure(code, message) {
  return Object.freeze({ schemaVersion: '0.1.0', type: 'benchmark-governance', status: 'failed', error: Object.freeze({ code, message }) });
}
