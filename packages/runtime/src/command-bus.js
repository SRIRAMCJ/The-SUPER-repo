...
export class RuntimeCommandBus {
  ...
  list() {
    return Object.freeze(this.operations.getOperations().map((operation) => Object.freeze({ ...operation, command: operation.id, input: inputSchema(operation.id) })));
  }
  ...
  async execute(command, input = {}, context = {}) {
    ...
    try {
      const authorized = await this.authorize({ command: definition, input, context, correlationId: correlation });
      if (!authorized) return failure('FORBIDDEN', `Runtime command denied: ${command}`, correlation, startedAt, this.clock());
    } catch (error) {
      return failure('AUTHORIZATION_ERROR', error instanceof Error ? error.message : String(error), correlation, startedAt, this.clock());
    }

    try {
      validateInput(command, input);
    } catch (error) {
      return failure('INVALID_INPUT', error instanceof Error ? error.message : String(error), correlation, startedAt, this.clock());
    }

    try {
      const data = await this.dispatch(command, input);
      return success(data, correlation, startedAt, this.clock());
    } catch (error) {
      return failure('COMMAND_FAILED', error instanceof Error ? error.message : String(error), correlation, startedAt, this.clock());
    }
  }
...
function validateInput(command, input) {
  if (command === 'runtime.cancel') {
    if (typeof input.executionId !== 'string' || !input.executionId.trim()) throw new TypeError('executionId must be a non-empty string');
    if (input.reason !== undefined && typeof input.reason !== 'string') throw new TypeError('reason must be a string');
  }
  if (input.limit !== undefined && (!Number.isInteger(input.limit) || input.limit < 1)) throw new TypeError('limit must be a positive integer');
  for (const key of ['executionId', 'type', 'status']) if (input[key] !== undefined && typeof input[key] !== 'string') throw new TypeError(`${key} must be a string`);
}
